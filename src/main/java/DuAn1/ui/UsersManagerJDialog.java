...
// Hàm kiểm tra email hợp lệ
    public boolean isValidEmail(String email) {
        ...
        sdf.setLenient(false); // Không cho phép sai kiểu (vd: 32/13/2024)

        try {
            Date date = sdf.parse(dateStr); // Nếu parse thành công thì hợp lệ
            return true;
        } catch (ParseException e) {
            return false; // Sai định dạng hoặc ngày tháng không tồn tại
        }
    }
